(() => {
  ...
  document.querySelectorAll('code[data-godbolt]').forEach((code,index) => {
    ...
    panel.innerHTML=`<label class="sr-only" for="${editorId}">${pair('Editable code · '+language,'可编辑代码 · '+language)}</label><textarea id="${editorId}" class="compiler-editor" spellcheck="false" autocapitalize="off" autocomplete="off" wrap="off"></textarea><div class="compiler-toolbar"><div><span class="compiler-provider">COMPILER EXPLORER</span><div class="compiler-config">${escape(name)}${flags?` <code>${escape(flags)}</code>`:''}</div></div><div class="compiler-actions"><button type="button" class="compiler-reset"><svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M3 4v6h6"></path><path d="M3 10a9 9 0 1 1 2 8"></path></svg><span class="sr-only">${pair('Reset example','恢复示例')}</span></button><button type="button" class="compiler-run"><svg viewBox="0 0 24 24" width="18" height="18" aria-hidden="true"><path class="run-play" d="M7 4 20 12 7 20Z" fill="currentColor"></path><circle class="run-spinner" cx="12" cy="12" r="8" fill="none" stroke="currentColor" stroke-width="2" stroke-dasharray="35 16"></circle></svg><span class="sr-only">${pair('Compile & run','编译并运行')}</span></button></div></div><p class="compiler-edited-note compiler-note" hidden>${pair('The article’s explanation describes the original example; results below come from your edited code. Reloading or switching examples resets your edits.','文章说明针对原始示例；下方结果来自你修改后的代码。刷新页面或切换示例会恢复示例代码。')}</p><div class="compiler-status" role="status" aria-live="polite" hidden></div><div class="compiler-results" hidden><section class="compiler-diagnostics"><h4>${pair('Compiler diagnostics','编译诊断')}</h4><pre class="compiler-output" tabindex="0"></pre></section><section class="compiler-runtime" hidden><h4>${pair('Standard output · stdout','标准输出 · stdout')}</h4><pre class="compiler-stdout" tabindex="0"></pre><h4>${pair('Standard error · stderr','标准错误 · stderr')}</h4><pre class="compiler-stderr" tabindex="0"></pre></section><details class="compiler-assembly" hidden><summary>${pair('Generated assembly','生成的汇编')}</summary><pre tabindex="0"></pre></details></div><p class="compiler-note">${pair('A successful run does not prove correctness or memory safety.','运行成功不代表代码始终正确或不存在内存安全问题。')}</p>`;
    ...
    function actionTitles(){
      const lang=document.documentElement.dataset.language;
      for(const [node,en,zh] of [[button,'Compile & run','编译并运行'],[restore,'Reset example','恢复示例']])node.title=lang==='en'?en:lang==='zh'?zh:`${en} / ${zh}`;
    }
    actionTitles();window.addEventListener('languagechange',actionTitles);
    ...
    editor.value=source;editor.setSelectionRange(0,0);editor.rows=Math.min(24,Math.max(8,source.split('\n').length+1));restore.disabled=true;
    ...
    function change(next){
      ...
      status.hidden=false;status.innerHTML=pair('Code modified · previous results cleared.','代码已修改 · 旧结果已清除。');
    }
    function syncExample(){const next=sourceText(code);if(next===baseline)return;baseline=next;editor.value=next;editor.setSelectionRange(0,0);change(next);restore.disabled=true;panel.querySelector('.compiler-edited-note').hidden=true;}
    ...
    restore.addEventListener('click',()=>{editor.value=baseline;editor.setSelectionRange(0,0);change(baseline);editor.focus();});
    button.addEventListener('click',async()=>{
      ...
      status.hidden=false;status.innerHTML=pair('Compiling and running on Godbolt…','正在 Godbolt 上编译并运行…');
      try{
